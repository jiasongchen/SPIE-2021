...
def train(model, device, optimizer, dataloader, epoch, arg):
    ...
    for batch_idx, (X, S) in enumerate(dataloader):
        ...
        M=poly_disk(S)        
        model.output='reg'
        Xn=pgd_attack(model, X, S, noise_norm, norm_type, max_iter, step, loss_fn=adv_loss(arg.adv_loss))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--noise_norm', default=0.01, type=float)
    ...
    parser.add_argument('--adv_loss', default='mae', type=str)#mse, mae
    ...
    #%%
    filename=('result/'+arg.net_name+'_disk_pca_regseg_advreg_'+arg.adv_loss
