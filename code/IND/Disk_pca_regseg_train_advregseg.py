...
from Disk_pca_regseg_train_advreg import adv_loss as adv_loss_reg
from Disk_pca_regseg_train_advseg import adv_loss as adv_loss_seg
#%%
...
def train(model, device, optimizer, dataloader, epoch, arg):
    ...
    for batch_idx, (X, S) in enumerate(dataloader):
        ...
        M=poly_disk(S)        
        model.output='reg'
        Xn1=pgd_attack(model, X, S, noise_norm, norm_type, max_iter, step, loss_fn=adv_loss_reg(arg.adv_loss_reg))
        model.output='seg'
        Xn2=pgd_attack(model, X, M, noise_norm, norm_type, max_iter, step, loss_fn=adv_loss_seg(arg.adv_loss_seg))
        model.output='reg_seg'
        Sp, Mp=model(X)
        Spn1, Mpn1=model(Xn1)
        Spn2, Mpn2=model(Xn2)
        loss_ce=(nnF.binary_cross_entropy_with_logits(Mp, M)
                 +0.5*nnF.binary_cross_entropy_with_logits(Mpn1, M)
                 +0.5*nnF.binary_cross_entropy_with_logits(Mpn2, M))/2
        Mp=torch.sigmoid(Mp)
        Mpn1=torch.sigmoid(Mpn1)
        Mpn2=torch.sigmoid(Mpn2)
        loss_dice=((1-dice(Mp, M, 'mean'))
                   +0.5*(1-dice(Mpn1, M, 'mean'))
                   +0.5*(1-dice(Mpn2, M, 'mean')))/2
        Sp=Sp.view(Sp.shape[0], -1, 2)
        Spn1=Spn1.view(Spn1.shape[0], -1, 2)
        Spn2=Spn2.view(Spn2.shape[0], -1, 2)
        loss_mae= ((S-Sp).abs().mean()
                   +0.5*(S-Spn1).abs().mean()
                   +0.5*(S-Spn2).abs().mean())/2
...
if __name__ == '__main__':
    ...
    parser.add_argument('--adv_loss_reg', default='mae', type=str)#mse, mae
    parser.add_argument('--adv_loss_seg', default='bce_dice', type=str)#lmb, bce, dice
    ...
    #%%
    filename=('result/'+arg.net_name+'_disk_pca_regseg_advregseg_'+arg.adv_loss_reg+'_'+arg.adv_loss_seg
