...
def plot_history(history):
    ...
    ax[0,0].set_ylim(0, 3)
    ax[0,0].set_yticks(np.arange(0, 3.5, step=0.5))
    ax[0,0].grid(True)
    ax[0,0].legend()
    
    ax[0,1].plot(history['mrse_val'], '-r', label='mrse_val')
    ax[0,1].set_ylim(0, 3)
    ax[0,1].set_yticks(np.arange(0, 3.5, step=0.5))
    ax[0,1].grid(True)
    ax[0,1].legend() 

    ax[0,2].plot(history['mrse_test'], '-r', label='mrse_test')
    ax[0,2].set_ylim(0, 3)
    ax[0,2].set_yticks(np.arange(0, 3.5, step=0.5))
    ax[0,2].grid(True)
    ax[0,2].legend() 

    ax[1,0].plot(history['loss4_train'], '-m', label='loss4_train')        
    ...
    ax[1,1].set_yticks(np.arange(0.9, 1.01, step=0.01))
    ax[1,1].grid(True)
    ax[1,1].legend()

    ax[1,2].plot(history['dice1_test'], '-r', label='dice1_test')
    ax[1,2].plot(history['dice2_test'], '-c', label='dice2_test')
    ax[1,2].set_ylim(0.9, 1)
    ax[1,2].set_yticks(np.arange(0.9, 1.01, step=0.01))
    ax[1,2].grid(True)
    ax[1,2].legend()
...
if __name__ == '__main__':
    ...
    #%%
    for epoch in tqdm(range(epoch_save+1, 100), initial=epoch_save+1, total=100):
