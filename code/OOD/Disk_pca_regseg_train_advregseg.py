...
def train(model, device, optimizer, dataloader, epoch, arg):
    ...
    for batch_idx, (X, S) in enumerate(dataloader):
        ...
        M=poly_disk(S)        
        model.output='reg'
        Xn1=pgd_attack(model, X, S, noise_norm, norm_type, max_iter, step, loss_fn=mae_loss)
        model.output='seg'
        Xn2=pgd_attack(model, X, M, noise_norm, norm_type, max_iter, step, loss_fn=bce_dice_loss)
        model.output='reg_seg_rec'
        Sp, Mp, Xp=model(X)
        Spn1, Mpn1, Xpn1=model(Xn1)
        Spn2, Mpn2, Xpn2=model(Xn2)
        loss_ce=(nnF.binary_cross_entropy_with_logits(Mp, M)
        ...
        loss_rec= ((X-Xp).abs().mean()
                   +0.5*(Xn1-Xpn1).abs().mean()
                   +0.5*(Xn2-Xpn2).abs().mean())/2
        loss=(loss_ce+loss_dice)/2+loss_mae+loss_rec
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        loss4_train+=loss_rec.item()
        if (batch_idx+1)%10 == 0:
            t_end=time.time()
            print('epoch', epoch, 'batch', batch_idx, 'time', t_end-t_start,
                  'loss_ce', loss_ce.item(), 'loss_dice', loss_dice.item(), 
                  'loss_mae', loss_mae.item(), 'loss_rec', loss_rec.item())
...
if __name__ == '__main__':
    ...
    #%%
    filename=('result/'+arg.net_name+'_disk_pca_regseg_advregseg'
